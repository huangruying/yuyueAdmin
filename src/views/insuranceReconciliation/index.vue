    ...
    deriveData(){
      if(this.lobbyData.data.length <= 0){
        this.$message({
          ...
          window.location.href = `${baseUrl}/wash/channelMonth/channelOrderMonthExport?pageNum=${this.lobbyData.current_page}&pageSize=${this.lobbyData.per_page}
          &orderNo=${orderNo}&couponCode=${couponCode}&licensePlate=${licensePlate}`
          // window.location.href = `${baseUrl}/wash/channelMonth/channelOrderMonthExport?pageNum=${this.lobbyData.current_page}&pageSize=${this.lobbyData.per_page}&orderNo=${orderNo}&couponCode=${couponCode}&licensePlate=${licensePlate}&channelId=${this.id}&month=${this.month}`
      }
    },
    open(id,remark) {
